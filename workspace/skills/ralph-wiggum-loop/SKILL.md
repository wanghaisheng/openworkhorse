...
execution_framework_integration:
  ralph_loop_execution:
    method: "execution_framework.executeRalphWiggumLoop()"
    automatic_triggering: true
    quality_gate_enforcement: true
    
  loop_steps_integration:
    - "run_all_checks": "运行所有检查（测试、lint、类型检查）"
    - "perform_self_review": "根据HARNESS.md进行自我审查"
    - "generate_improvements": "生成改进建议"
    - "calculate_quality_score": "计算质量分数"
    - "update_wbs_status": "更新WBS里程碑状态"
    - "validate_openspec": "验证OpenSpec一致性"
...
input:
  ...
  loop_config: object          # 循环配置（可选，默认使用ExecutionFramework配置）
  trigger_event: string        # 触发事件
  quality_targets: map[string]float  # 质量目标（可选，从HARNESS.md解析）
  auto_improvement: boolean     # 自动改进开关
  use_framework_loop: boolean  # 使用ExecutionFramework的Ralph Loop
```
