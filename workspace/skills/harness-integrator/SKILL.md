...
execution_framework_integration:
  harness_rules_loading:
    method: "execution_framework.loadHarnessRules()"
    auto_parsing: true
    fallback_to_default: true
    
  quality_gate_enforcement:
    test_coverage_threshold: "从HARNESS.md解析"
    code_quality_threshold: "从HARNESS.md解析"
    security_score_threshold: "从HARNESS.md解析"
    performance_score_threshold: "从HARNESS.md解析"
    
  violation_detection:
    prohibition_checking: true
...
input:
  ...
  harness_path: string         # HARNESS.md文件路径（可选，默认使用ExecutionFramework配置）
  check_type: string           # 检查类型：code/architecture/process
  strict_mode: boolean         # 严格模式检查
  auto_fix: boolean           # 自动修复简单违规
  use_framework_config: boolean # 使用ExecutionFramework配置
```
